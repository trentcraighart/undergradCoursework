...
maxInt :: Tree -> Int
maxInt (Leaf i)     = i
maxInt (Node a b c) = max a (max (maxInt b) (maxInt c))
...
minInt :: Tree -> Int
minInt (Leaf i)     = i
minInt (Node a b c) = min a (min (minInt b) (minInt c))
...
sumInts :: Tree -> Int
sumInts (Leaf i)   = i
sumInts (Node a b c) = a + (sumInts b) + (sumInts c)
...
isBST :: Tree -> Bool
isBST (Leaf i)     = True
isBST (Node a b c) = ((isBST(b)) && (isBST(c)) && ((returnVal(b)) <= a) && (a <= returnVal(c))) 
...
inBST :: Int -> Tree -> Bool
inBST j (Leaf i)     = (i == j)
inBST j (Node a b c) = (a == j) || (inBST j b) || (inBST j c)



-- PERSONAL FUNCTIONS BY TRENT VASQUEZ -- 
returnVal :: Tree -> Int
returnVal (Leaf i)     = i
returnVal (Node a _ _) = a
