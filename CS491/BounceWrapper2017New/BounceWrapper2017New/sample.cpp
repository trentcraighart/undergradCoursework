...
Bounce( float dt )
{
	while( dt > EPSILON )
	{
		...
		if (Vxnow > 0) {
			distance = XRIGHT - Xnow - RADIUS;
			tleft = distance / Vxnow;
		}
		...
		if (Vxnow < 0) {
			//tright = - Radius;
			distance = Xnow - XLEFT - RADIUS;
			tright = distance / (-Vxnow);
		}
		...
		distance = Ynow - YBOTTOM - RADIUS;

		printf("distance %f\n", distance);

		tfloor1 = (-b + sqrt(pow(b, 2) - (2 * a) * (distance))) / a;
		tfloor2 = (-b - sqrt(pow(b, 2) - (2 * a) * (distance))) / a;
		//tfloor2 = (distance / ((-b - (sqrt((pow(b, 2)) - (4.0 * a * c)))) / (2.0 * a)));


		//printf("tleft%f\ntright%f\ntfloor1 %f\ntfloor2 %f\n", tleft, tright, tfloor1, tfloor2);
