...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    //Value is bigger than or equal to root
		...
		if(cur->right == 0){
			...
			if(cur->left == 0){
				...
				*cur = *cur->right;
				...
				cur->right = _removeNode(cur->right, cur->right->val);
				return cur;
			}
		}
	}
	if(test == -1){
		if(cur->right != 0 && cur->right != NULL){
			cur->right = _removeNode(cur->right, val);
		}
		return cur;
	}
	if(cur->left != 0 && cur->left != NULL){
...
struct TestValues* createValues2()
{
    struct TestValues* values = malloc(sizeof(struct TestValues));
    values->n = 12;
    values->values = malloc(values->n * sizeof(struct data));
    
    initValue(&(values->values[0]), 15, "rooty");
    initValue(&(values->values[1]), 13, "n");
    initValue(&(values->values[2]), 5, "n");
    initValue(&(values->values[3]), 4, "n");
    initValue(&(values->values[4]), 8, "n");    
    initValue(&(values->values[5]), 19, "n");
    initValue(&(values->values[6]), 22, "n");
    initValue(&(values->values[7]), 16, "n");
    initValue(&(values->values[8]), 55, "n");
    initValue(&(values->values[9]), 1, "n");
    initValue(&(values->values[10]), 28, "n");
    initValue(&(values->values[11]), 7, "n");
    return values;
...
void testContainsBSTree2()
{
    struct TestValues* tv = createValues2();
    struct BSTree *tree = newBSTree();
    ...
    int i, j;
    j = 0;
    i = containsBSTree(tree, &(tv->values[j]));
    printf("Return val = %d for element %d\n", i, j);
    j++;
    i = containsBSTree(tree, &(tv->values[j]));
    printf("Return val = %d for element %d\n", i, j);
    j++;
    ...
    int fakevallocal;
    fakevallocal = 17;
    fakeval = &fakevallocal;
    i = containsBSTree(tree, fakeval);
    printf("Return val = %d for fake value 17\n", i);                       
    deleteBSTree(tree);
    destroyValues(tv);
}
/**
...
void testRemoveNode2(){
    ...
    tree->root = _removeNode(tree->root, &(tv->values[6]));
    --(tree->cnt);
    ...
    tree->root = _removeNode(tree->root, &(tv->values[9]));
    --(tree->cnt);
    ...
    tree->root = _removeNode(tree->root, &(tv->values[11]));
    --(tree->cnt);
