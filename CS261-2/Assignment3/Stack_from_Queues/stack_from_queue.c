...
void listSwapStackQueues(struct Stack* stack)
{
	...
	struct Link* temp;
	temp = stack->q1->head;
	stack->q1->head = stack->q2->head;
	stack->q2->head = temp;
	temp = stack->q1->tail;
	stack->q1->tail = stack->q2->tail;
	stack->q2->tail = temp;	
...
void listStackPush(struct Stack* stack, TYPE value)
{
	...
	while(listStackIsEmpty(stack) == 0){
